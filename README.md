...
**Material disponible** en [`docs/defensa/`](docs/defensa/) y [`docs/predeposito/`](docs/predeposito/): memoria del TFM (docx + pdf), presentaciones (defensa + divulgativa) con vídeos y QR, y guion completo de defensa.
...
- **Iter 7b** (best-of-N): diagnóstico de Iter 7a → 12/21 fallos de grasp eran *borderline* (<8 cm, umbral 5 cm), drift de precisión, no fallo de IK. Se muestrean **N=8 trayectorias** y se ejecuta la de menor proximidad de grasp al objeto (pose conocida vía FoundationPose). **Sin reentrenar, coste de sim idéntico.** Recupera el grasp (58→82 %) y **supera al baseline supervisado en éxito E2E: 78 % vs 60 %** (luego elevado a 84 % por el fix de IK de Iter 7c). La ablación N∈{1,4,8,16} confirma N=8 como punto óptimo: el éxito E2E se satura ahí (N=16 no mejora, 76 %) porque la selección por proximidad de grasp empieza a penalizar IK/deposit. Ver [`docs/INTEGRATION_PIPELINE.md`](docs/INTEGRATION_PIPELINE.md).
- **Iter 7c** (fix de IK): la convergencia de IK era frenada por una semilla divergente del solver (no por reach ni singularidad — refutados con evidencia). Re-sembrar el IK desde la config home al fallar lleva la **convergencia de IK del 76 % al 100 %** y el éxito E2E del 78 % al **84 %** (comparación pareada con misma semilla, sin regresión en grasp). Aplica a todo el pipeline.
