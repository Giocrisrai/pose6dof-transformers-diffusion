#!/usr/bin/env python3
"""Eval Iter 5b — DP v5 con deposit phase, single-object.

50 picks (seed=2026) usando pick_with_dp con attach/release automático.
Mide grasp_plausible + deposit_plausible (que en Iter 1-4 fue 0%).

Uso (CoppeliaSim running on :23000):
    python experiments/eval_diffusion_iter5_sim.py --n 50
"""
...
logger = logging.getLogger("eval_iter5_sim")
...
def main() -> int:
    ...
    policy_path = REPO / "data" / "models" / "diffusion_policy_sim_v5.pth"
    encoder_ckpt = REPO / "data" / "models" / "visual_encoder_iter5.pth"
    if not policy_path.exists() or not encoder_ckpt.exists():
        logger.error("policy o encoder v5 no encontrados")
        return 1
    ...
    logger.info(f"policy: v5 (hidden_dim={hd}) + ResNet-18")
    ...
    pick_and_place = sum(
        1 for r in results if r["grasp_plausible"] and r["deposit_plausible"]
    )
    ...
    summary = {
        ...
        "pick_and_place_success_pct": 100.0 * pick_and_place / n_valid,
    ...
    out = REPO_OUT / "eval_v5_sim.json"
    out.write_text(json.dumps(summary, indent=2))
    print("\n=== RESUMEN EVAL ITER 5 (con deposit) ===")
