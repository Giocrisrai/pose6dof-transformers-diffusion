...
def build_decision_card(pred, dataset, model, scheduler, device):
    ...
    phases = [
        ...
        ("Diffusion",      "planning",    diff_ms,        '#0F766E'),
        ("Simulacion",     "CoppeliaSim", NOMINAL_SIM_MS, '#A33D17'),
    ]
    bar_y = 0
    cumulative = 0
    # Estrategia simple y robusta: dentro de la barra ponemos SOLO el tiempo
    # en bold blanco. El nombre va siempre fuera, en una leyenda lateral
    # con anclas de color. Asi nunca se solapa el nombre con las barras
    # vecinas, sea cual sea la proporcion entre fases.
    for label, sub, dur, c in phases:
        axD.barh(bar_y, dur, left=cumulative, color=c,
                  edgecolor='white', linewidth=2, height=0.55, zorder=2)
        # Dentro: solo la duracion (corto, siempre cabe si la barra es razonable)
        if dur >= total_ms * 0.04:  # > 4 % del total
            axD.text(cumulative + dur / 2, bar_y,
                      f"{dur:.0f} ms",
                      ha='center', va='center', fontsize=11,
                      fontweight='bold', color='white', zorder=3)
        cumulative += dur

    # Leyenda lateral con los nombres y duraciones (siempre legible)
    legend_x = max(11500, cumulative * 1.25) * 0.62  # zona derecha del panel
    legend_y = 0.78
    for i, (label, sub, dur, c) in enumerate(phases):
        y = legend_y - i * 0.22
        axD.add_patch(plt.Rectangle((legend_x, y - 0.05), total_ms * 0.025, 0.10,
                                      facecolor=c, edgecolor='white',
                                      linewidth=1, zorder=4))
        axD.text(legend_x + total_ms * 0.030, y,
                  f"{label} ({sub}) — {dur:.0f} ms",
                  va='center', fontsize=10, color='#0F172A',
                  fontweight='bold', zorder=4)

    # Limite H3 + total
    axD.axvline(10000, color='#A91D1D', linestyle='--', linewidth=2, alpha=0.85)
    axD.text(10000, -0.55, 'Limite H3 (10 s)', fontsize=10, color='#A91D1D',
              fontweight='bold', ha='center')

    veredict_color = '#0F7A37' if total_ms < 10000 else '#A91D1D'
    veredict_txt = 'OK' if total_ms < 10000 else 'EXCEDE'
    axD.text(cumulative * 0.5, -0.70,
              f"TOTAL {total_ms:.0f} ms  ·  {veredict_txt} ({total_ms/10000:.0%} del limite)",
              ha='center', fontsize=12, fontweight='bold', color=veredict_color)

    axD.set_xlim(-200, max(11500, cumulative * 1.25))
    axD.set_ylim(-0.95, 0.95)
    axD.set_xlabel('Tiempo (ms)', fontsize=10, fontweight='bold')
    axD.set_yticks([])
