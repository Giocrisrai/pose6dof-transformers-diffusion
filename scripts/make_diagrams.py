...
def styled_node(g, name, label, color, text_color="white", subtitle=None):
    ...
    if subtitle:
        ...
        html_label = f"<<font face='Helvetica-Bold' point-size='15' color='{text_color}'><b>{label}</b></font>>"
...
def diag10_pipeline_full():
    ...
    g.attr(
        label=(
            "<<font point-size='20'><b>Pipeline de Bin Picking 6-DoF</b></font><br/>"
    ...
    styled_node(
        g, "fp", "FoundationPose", COLORS["primary"], subtitle="Transformer cross-attn 2D-3D"
    )
    styled_node(
        g,
        "freeze",
    ...
    with g.subgraph(name="cluster_vla") as c:
        ...
        styled_node(
            c, "clip_i", "CLIP-image (frozen)", COLORS["purple"], subtitle="768-D embedding"
        )
        styled_node(
            c,
            "gate",
            "TextGroundedGate / VisualGate",
            COLORS["cyan"],
...
def diag11_vla_lite_flow():
    ...
    g.attr(
        label=(
            "<<font point-size='20'><b>VLA-lite — Flujo end-to-end</b></font><br/>"
            "<font point-size='12' color='#64748B'>"
    ...
    with g.subgraph(name="cluster_input") as c:
        c.attr(
            ...
            fontcolor=COLORS["ink_soft"],
            fontname="Helvetica-Bold",
            fontsize="13",
        )
        styled_node(c, "instr", "Instruccion", COLORS["amber"], subtitle="'pick the red sphere'")
        styled_node(
            c, "scene", "Escena (RGB)", COLORS["muted"], subtitle="cam cenital + segmentacion"
        )

    # CLIP encoders (ambos frozen)
    with g.subgraph(name="cluster_clip") as c:
        c.attr(
            label="CLIP (frozen · 150 M params)",
            style="rounded,filled",
            fillcolor="#F3E8FF",
            color=COLORS["purple"],
            penwidth="2.5",
        ...
        styled_node(c, "text_enc", "CLIPTextModel", COLORS["purple"], subtitle="output 512-D")
        styled_node(c, "vis_enc", "CLIPVisionModel", COLORS["purple"], subtitle="output 768-D")

    # Gate entrenable
    with g.subgraph(name="cluster_gate") as c:
        c.attr(
            label="MODULOS ENTRENABLES (~150 K params)",
            style="rounded,filled",
            fillcolor="#CCFBF1",
            color=COLORS["teal"],
            penwidth="2.5",
    ...
    # Resultado
    styled_node(g, "gates", "Gates por objeto", COLORS["success"], subtitle="probabilidades 0..1")
    styled_node(g, "sel_pos", "Selected position", COLORS["accent"], subtitle="weighted average")
    styled_node(g, "diff", "Diffusion Policy", COLORS["primary"], subtitle="DDIM-25 condicionado")
    styled_node(g, "traj_out", "Trayectoria final", COLORS["ink"], subtitle="16 pasos × 7 DoF")
...
def diag12_exploraciones_dag():
    ...
    g.attr(
        label=(
            "<<font point-size='20'><b>13 Exploraciones post-TFM — Dependencias</b></font><br/>"
            "<font point-size='12' color='#64748B'>"
...
def diag13_data_flow():
    ...
    g.attr(rankdir="TB", bgcolor="white", pad="0.5", nodesep="0.4", ranksep="0.7", splines="spline")
    g.attr("node", **COMMON_NODE_ATTR)
    g.attr("edge", **COMMON_EDGE_ATTR)

    g.attr(
        label=(
            "<<font point-size='20'><b>Flujo de datos en runtime — Tipos y dimensiones</b></font><br/>"
            "<font point-size='12' color='#64748B'>"
    ...
    # Outputs (verde claro)
    data_node(
        "pose_out", "Pose 6-DoF", "R en SO(3), t en mm", fill="#DCFCE7", border=COLORS["success"]
    )
    data_node(
        "emb_out", "CLIP embedding", "512-D / 768-D", fill="#DCFCE7", border=COLORS["success"]
    )
    data_node("gates_out", "Gates softmax", "N × float", fill="#DCFCE7", border=COLORS["success"])
    data_node("traj_out", "Trayectoria", "16 × 7 floats", fill="#DCFCE7", border=COLORS["success"])
    data_node(
        "ctrl_out", "Comando articular", "6-DoF joint pos", fill="#DCFCE7", border=COLORS["success"]
    )
